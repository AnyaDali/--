...
public:
    ...
    void save(std::string &&f_name)
    {
        ...
        std::string path = "saves/" + f_name + ".txt";
        std::ofstream buff(path.c_str());
        for (const auto &el : __gv.get_vec_ptr())
        {
            buff << (*el).point.first << " " << (*el).point.second << " ";
        }
        buff << "#";
        auto ptr = __gv.lines_data();
        for (size_t i = 0; i < __gv.lines_size(); ++i)
        {
            buff << *ptr << " ";
            ++ptr;
        }
    ...
    {
        std::string path = "saves/" + f_name + ".txt";
        std::ifstream buff(path.c_str());
        if (!buff.is_open())
        {
            std::cout << "file " << f_name << "is not opened\n";
            return;
        }
        clear();
        // читать вершины
