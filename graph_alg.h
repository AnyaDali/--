...
public:
    ...
    void DSATUR_algorithm()
    {
        ...
        std::vector<int> d(n); // с(n) - вектор цветов, d(n) - кол-во смежных вершин
        std::vector<std::set<int>> adjCols(n, std::set<int>());
        std::set<graph_algorithm<size_sq, _Alloc>::nodeInfo, graph_algorithm<size_sq, _Alloc>::maxSat> Q;
        auto maxPtr = Q.begin();
        ...
        for (u = 0; u < n; u++)
        {
            ...
            Q.emplace(nodeInfo{0, d[u], u}); // заполения дерева
        }
    ...
    void color_the_graph()
    {
        ...
        for (std::size_t i = 0; i < colorArr.size(); ++i)
        {
            ...
            vertexVisual<size_sq, _Alloc>::change_the_color_of_the_vertex(i, R, G, B);
        }
    }
